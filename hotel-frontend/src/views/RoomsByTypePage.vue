...
<MainNavbar />
  ...
  <div id="allRooms">
    <h1 id="h1Rooms">All {{ typetitle }}</h1>
    ...
    <input type="date" id="arrival" v-model="params.arrivalDate" required @change="sortByDate"/>
    <label id="arrivalLabel" for="departure">Departure Date</label>
    <input type="date" id="departure" v-model="params.departureDate" required @change="sortByDate"/>
    ...
    <div class="shwMoreButton" v-if="visibleRooms.length < rooms.length">
...
export default defineComponent({
  ...
  setup() {
    ...
    const getRoomsByType = async (
      ...
      try {
        const response = await RoomService.getAllByType(
          typetitle,
          status,
          direction,
          arrivalDate,
    ...
    getRoomsByType(typetitle, "", "", null, null);
    ...
    const sortRooms = () => {
      getRoomsByType(typetitle, params.status, params.direction, params.arrivalDate, params.departureDate);
    };
